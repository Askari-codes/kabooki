...
const BookProfile =async ({params}:{params:{id:string}}) => {
    ...
    const response = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/api/books/${params.id}`)
  ...
  return (
    ...
    >
      ...
      >
        <Box>
          <Image
            src={"/"+book.picture_url+'jpg' || "/default-book.jpg"} // Default image if picture_url is null
            alt={book.title}
            width={200}
            height={300}
            style={{
      ...
      <Box>
        ...
        <Separator />
        <Text as="p" size="2" style={{ color: "var(--gray-700)", lineHeight: "1.6" }}>
          Lorem ipsum dolor sit amet, consectetur adipiscing elit. Quisque sit amet accumsan
          tortor. Morbi hendrerit sapien ut est fringilla, nec fringilla magna pellentesque.
          Praesent id nisi eget metus blandit mollis a at erat.
        </Text>
      </Box>
    </Box>
  );
